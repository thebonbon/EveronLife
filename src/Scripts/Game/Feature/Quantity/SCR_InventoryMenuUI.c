	...
	override void NavigationBarUpdateGamepad()
	{
		if (m_pSelectedSlotUI)
		{
			...
			if (quantitySource)
			{
				...
				bool isValidTargetStorage = isFocusStorage && m_pActiveStorageUI && !EL_Utils.IsAnyInherited(m_pActiveStorageUI.Type(), {SCR_InventoryStoragesListUI, SCR_InventoryStorageWeaponsUI});
